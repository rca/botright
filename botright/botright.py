...
class Botright(AsyncObject):
    ...
    async def new_browser(self, proxy: Optional[str] = None, flags: Optional[list] = None, **launch_arguments) -> BrowserContext:
        """
        ...
        if self.mask_fingerprint:
            flags += [f"--user-agent={_faker.fingerprint.navigator.user_agent}"]
